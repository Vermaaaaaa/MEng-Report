...
\subsection{Datasets and Experimental Data}
The dataset used for UKF evaluation has been from BROAD \cite{laidig_2021_broada} whilst training, testing and, validation data will be from a range of sources
including BROAD, OxIOD \cite{a2019_dataset}, and others to improve 
generalisation of the model across sensors, motion scenarios, and disturbances. 

The reason why BROAD
was used for the baseline is that it has many appealing qualities, these 
being isolated magnetometer and accelerometer disturbances, short and long motions, and rest phases.
These properties allow us to estimate the noise of each sensor for process and measurement noise 
covariances, explored in \hyperref[sec:filtersetup]{section 3.3},  from the rest segments and tests the
filter under disturbed conditions.
...
a heuristic approach was taken by tuning arbitrary tolerances to accept or reject the sensor
measurements. If measurements was deemed unreliable, then the matrix $R$ would be multiplied by a scalar
value to downweight updates during corrections, this scheme was selected as by outright rejecting measurements caused 
jerking orientation estimates.

The matrix $Q$, also took a similar approach where gyro bias was determined through subtracting 
the mean during rest phases and these resdiuals were computed as the covariance which was modelled to be Gaussian noise. 
\newpage
...
were close to ideal. The second scenario for how high linear accelerations corrupt orientation
estimates (inclination) given by the filter, and the final scenario for how magnetometer disturbances
...
with a $\mu = 5.8150$, $\sigma = 3.7535$ $\text{median} = 4.9821$. This means that under undisturbed conditions of the
...
with a $\mu = 11.4535$, $\sigma = 9.0682$ $\text{median} = 9.0273$. This means that under the
disturbed conditions of the accelerometer, it can be seen that the UKF is a reasonable choice, 
...
with a $\mu = 37.8035$, $\sigma = 38.8809$ $\text{median} = 23.6325$. This means that under
a magnetic disturbance, a UKF alone cannot be used for accurace orientation estimates even with
...
through deep-learning then accurate orientation estimates from an IMU is unlikely.
