...
vision tasks; it has seen wide use in research relating to drift reduction \cite{chen_2018_improving}\cite{brossard_2020_denoising}. 
...
trained by using parallelisation \cite{krizhevsky_2012_imagenet}. As stated before, RNNs cannot exploit 
...
\centering
\small
\begin{tabular}{p{2.0cm} p{5cm} p{7.5cm} }
\hline
\textbf{Method} & \textbf{Normalises}   & \textbf{Pros / Cons} \\
\hline
...
to our output. This is because seen in equations 10,12,14, the relationship between
weights and input data is all linear. If the relationship between input and output
were linear there would be no need for a deep learning model as these relationships 
could be modelled easily. The table below demonstrates common activation functions
used.
...
\begin{tabular}{p{2.0cm} p{2.0cm} p{2.0cm} p{3.5cm} p{5.0cm}}
\hline
\textbf{Activation} & \textbf{Definition} & \textbf{Output range} & \textbf{When used} & \textbf{Pros / Cons} \\
\hline
