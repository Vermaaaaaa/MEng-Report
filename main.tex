\documentclass{article}
\usepackage{graphicx}
\usepackage[backend=biber,style=ieee]{biblatex}
\usepackage[a4paper,margin=2.5cm]{geometry} 
\usepackage{xcolor}
\usepackage{gensymb} % For \degree
\usepackage{bm}
\usepackage{tabularx}
\usepackage{parskip}
\usepackage{float}
\usepackage{tikz}
\usepackage{placeins}
\usepackage{amsmath}
\usepackage{makecell}
\usepackage{comment}
\usepackage{tikz}
\usepackage{multirow}
\usepackage{booktabs}
\usepackage{pdfpages}
\usetikzlibrary{positioning, fit, arrows.meta, shadows, calc}
\usetikzlibrary{positioning, shadows, arrows.meta}
\usetikzlibrary{positioning} % Required for 'below=of' syntax
\usetikzlibrary{shapes.geometric} % Required for the 'diamond' shape
\usetikzlibrary{positioning,fit,arrows.meta}
\addbibresource{./bib/references.bib}




\newcommand{\todo}[1]{\textcolor{red}{\textbf{TODO:} #1}}
\newcommand{\fixme}[1]{\textcolor{orange}{\textbf{FIXME:} #1}}
\newcommand{\words}[1]{\textcolor{blue}{\textbf{WORDS:} #1}}

\usepackage[hidelinks]{hyperref}


\graphicspath{{figures/}}


\begin{document}

\input{sections/FrontPages/titlepage}

\includepdf[pages=-]{sections/declaration/ELEC5870M_Declaration_of_Academic_Integrity.pdf}

\input{sections/FrontPages/abstract}

\input{sections/FrontPages/acknowledgements}

\newpage

\tableofcontents

\input{sections/FrontPages/LoF}

\input{sections/FrontPages/LoT}



\input{sections/Intro/introduction}

\input{sections/Societal_Impacts/societal}



\input{sections/IMU_background/imu}

\input{sections/Datasets/dataset}

\input{sections/Filter/mukf}

\input{sections/Neural/model}

\input{sections/loss_fn/loss_fn}


\input{sections/Results_Evaluation/combined}

\input{sections/Conclusion/conclusion}

\newpage
\printbibliography



\end{document}
